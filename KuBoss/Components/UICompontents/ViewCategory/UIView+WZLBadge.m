...
- (void)kbShowBadgeWithStyle:(WBadgeStyle)style
               ...
               animationType:(WBadgeAnimType)aniType
{
    ...
    switch (style) {
        case WBadgeStyleRedDot:
            ...
            break;
        case WBadgeStyleNumber:
            [self kbshowNumberBadgeWithValue:value];
            break;
        case WBadgeStyleNew:
            [self showNewBadge];
            break;
...
- (void)kbshowNumberBadgeWithValue:(NSInteger)value
{
    if (value < 0) {
        return;
    }
    ...
    self.badge.frame = frame;
    self.badge.center = CGPointMake(CGRectGetWidth(self.frame) - 0 + self.badgeCenterOffset.x, self.badgeCenterOffset.y+5);
...
- (void)showNumberBadgeWithValue:(NSInteger)value
{
    ...
    self.badge.frame = frame;
    self.badge.center = CGPointMake(CGRectGetWidth(self.frame) - 8 + self.badgeCenterOffset.x, self.badgeCenterOffset.y+5);
    self.badge.layer.cornerRadius = CGRectGetHeight(self.badge.frame) / 2;
}
